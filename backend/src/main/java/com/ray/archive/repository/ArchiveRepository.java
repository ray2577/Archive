...
public interface ArchiveRepository extends JpaRepository<Archive, Long>, JpaSpecificationExecutor<Archive> {
    ...
    @Query("SELECT a.type, COUNT(a) FROM Archive a GROUP BY a.type")
    List<Object[]> countByTypes();

    @Query("SELECT FUNCTION('FORMAT', a.createTime, 'yyyy-MM') as month, COUNT(a) FROM Archive a GROUP BY month ORDER BY month DESC")
    List<Object[]> countByMonths();

    @Query("SELECT a FROM Archive a WHERE " +
           "(:code IS NULL OR a.fileNumber LIKE %:code%) AND " +
           "(:name IS NULL OR a.title LIKE %:name%) AND " +
           "(:type IS NULL OR a.type = :type) AND " +
           "(:status IS NULL OR a.status = :status) AND " +
           "(a.deleted = false)")
    Page<Archive> findByFilters(@Param("code") String code, 
